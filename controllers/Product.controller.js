  ...
  try {
    ...
    for (const product of products) {
      ...
      if (!supplier) {
        return res
          .status(400)
          .json({ message: `Supplier with email ${email} not found` });
      }
      ...
      if (!categoryDoc) {
        return res
          .status(400)
          .json({ message: `Category with name ${category} not found` });
      }
...
const getAllProductsCategory = async (req, res) => {
  try {
    const products = await Product.find().populate("category"); // This will populate the category data
  ...
  try {
    ...
    if (supplier) {
      supplier.productsSupplied = supplier.productsSupplied.filter(
        (productInfo) =>
          productInfo.productId.toString() !== product._id.toString()
      );
      await supplier.save();
    }
